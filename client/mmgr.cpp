...
Mmgr::Mmgr() {
	...
	if (kernel_base && kernel_size) {
		m_comms_state.event_um = CreateEvent(NULL, FALSE, FALSE, NULL);
		m_comms_state.event_km = CreateEvent(NULL, FALSE, FALSE, NULL);
		m_comms_state.timeout = 300000;
		m_comms_thread = std::thread(comms_init, kernel_base, kernel_size, &m_comms_state);
	}
}
...
bool Mmgr::attach(uint32_t process_id) {
	...
	m_process = comms_get_process(process_id, &m_comms_state);
...
void Mmgr::detach() {
	...
	if (m_process) {
		comms_dereference(m_process, &m_comms_state);
		m_process_id = 0;
		m_process = 0;
	}
}
...
void Mmgr::stop() {
	...
	detach();
	comms_exit(&m_comms_state);
	if (m_comms_thread.joinable()) {
		m_comms_thread.join();
	}

	CloseHandle(m_comms_state.event_um);
	CloseHandle(m_comms_state.event_km);
}

void Mmgr::get_module(const wchar_t* module, void** module_base, size_t* module_size) {
	comms_get_module(m_process, module, module_base, module_size, &m_comms_state);
}

void* Mmgr::find_pattern(void* start, size_t size, const char* pattern, const char* mask) {
	return comms_find_pattern(m_process, start, size, pattern, mask, &m_comms_state);
}
...
void Mmgr::mem_alloc(void** base, size_t* size, uint32_t type, uint32_t protect) {
	comms_mem_alloc(m_process, base, size, type, protect, &m_comms_state);
}
...
void Mmgr::mem_free(void** base, size_t* size, uint32_t type) {
	comms_mem_free(m_process, base, size, type, &m_comms_state);
}
...
bool Mmgr::replace_ptes(void* src, void* dst, size_t size, void* original) {
	return comms_replace_ptes(m_process, src, dst, size, original, &m_comms_state);
}
...
bool Mmgr::restore_ptes(void* base, size_t size, void* original) {
	return comms_restore_ptes(m_process, base, size, original, &m_comms_state);
}
...
void* Mmgr::duplicate_handle(void* handle, uint32_t access, uint32_t options) {
	return comms_duplicate_handle(m_process, handle, access, options, &m_comms_state);
}

void Mmgr::close_handle(void* handle) {
	comms_close_handle(m_process, handle, &m_comms_state);
}
